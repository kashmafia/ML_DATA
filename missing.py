...
filename = open("/home/kblanco2/machine-learning-project/TrainData3.txt", "r")
...
def calculate_missing_value(dataset, distances, tuple_i):
    ...
    # contains the k + 1 lowest distances from the inputted tuple's distance list
    nearest = sorted(tuple_dist)[:k]
    # contains the indexes of the k nearest neighbors to the inputted tuple
    neighbors = []
    # contains the indexes of the missing values of the inputted tuple
    missing_index = []

    # finds the indexes of the k nearest tuples in the dataset as n is equal to 3 in this implementation
    ...
    for i in range(len(missing_index)):
        ...
        if values != 0:
            # the mean of the values of the k nearest neighbors that are at the same index of the missing index is calculated
...
with open("TrainData3Updated.txt", "w") as file:
    for item in dataset:
